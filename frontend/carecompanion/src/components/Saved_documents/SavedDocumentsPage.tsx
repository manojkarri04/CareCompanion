...
const fetchDocuments = async () => {
    try {
      ...
      // 3. Format the dates
      const formattedData = data.map((doc: { upload_date?: string; uploadDate?: string; [key: string]: unknown }) => ({
        ...doc,
        // Make sure we use upload_date because Supabase uses underscores!
        uploadDate: new Date((doc.upload_date || doc.uploadDate) as string) 
      })) as Document[];
  ...
  const handleView = async (fileName: string) => {
    try {
      ...
      const apiUrl = API_URL;
      const response = await fetch(`${apiUrl}/api/documents/file/${fileName}`, {
        headers: { 'Authorization': `Bearer ${session?.access_token}` }
      });
  ...
  return (
    <DashboardLayout activePage="saved-docs">
