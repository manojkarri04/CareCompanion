...
def get_embedding(text: str) -> List[float]:
    ...
    # Fallback deterministic pseudo-embedding (384 dimensions) if sentence-transformers not initialized
    import hashlib
    vec = []
...
def store_document_chunks(db, user_id: str, document_id: str, filename: str, full_text: str) -> bool:
    """Extracts chunks from document text, computes 384-dim embeddings, and inserts into Supabase document_embeddings."""
...
def store_profile_embedding(db, user_id: str, full_name: str, email: str) -> bool:
    """Stores user profile metadata as a vector chunk in document_embeddings."""
    if not db:
        return False
    try:
        ...
        db.table('document_embeddings').insert(record).execute()
        return True
    except Exception as e:
...
def similarity_search(db, user_id: str, query: str, top_k: int = 5, match_threshold: float = 0.1) -> List[Dict[str, Any]]:
    ...
    try:
        # Call the Supabase match_document_chunks RPC
    ...
    try:
        # Fallback to direct query if RPC is not enabled yet
